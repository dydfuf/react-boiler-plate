...
function Chatbot() {
    ...
    const renderCards = (cards) => {
        ...
        return cards.map((card, index) => <Card key={index} cardInfo={card.structValue} />)
    ...
    const renderOneMessage = (message, i) => {
        ...
        // we need to git some condition here to separate message kinds

        // template for normal text

        if (message.content && message.content.text && message.content.text.text) {
            return <Message key={i} text={message.content.text.text} who={message.who} />
        } else if (message.content && message.content.payload.fields.card) {

            const AvatarSrc = message.who === 'bot' ? <RobotOutlined /> : <SmileOutlined />

            return <div >
                <List.Item style={{ display: 'inline', padding: '1rem' }}>
                    <List.Item.Meta
